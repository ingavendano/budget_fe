...
export class ExpenseComponent implements OnInit {
  ...
  saveExpense() {
    ...
    this.configService.saveExpense(newExpense).subscribe({
      next: () => {
        this.authService.updateOnboarding(5).subscribe();
  ...
  exportToPdf() {
    this.configService.exportExpensesPdf().subscribe((blob: Blob) => {
      this.downloadFile(blob, 'egresos.pdf');
    });
  }

  exportToExcel() {
    this.configService.exportExpensesExcel().subscribe((blob: Blob) => {
