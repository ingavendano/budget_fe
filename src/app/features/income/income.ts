...
export class Income implements OnInit {
  ...
  saveIncome() {
    ...
    this.configService.saveIncome({
      ...
      this.authService.updateOnboarding(4).subscribe();
      Swal.fire({
        icon: 'success',
  ...
  exportToPdf() {
    this.configService.exportIncomesPdf().subscribe((blob: Blob) => {
      this.downloadFile(blob, 'ingresos.pdf');
    });
  }

  exportToExcel() {
    this.configService.exportIncomesExcel().subscribe((blob: Blob) => {
